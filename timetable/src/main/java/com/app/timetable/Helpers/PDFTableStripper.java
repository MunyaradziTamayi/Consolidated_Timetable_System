...
//        System.err.println("Drawing " + nCols + "x" + nRows + "="+ nRows*nCols + " regions");
    ...
    private ArrayList<Rectangle2D> getFirstColumnBoxes() {
        double min = findMinimumX();
        ArrayList<Rectangle2D> firstColumn=new ArrayList<Rectangle2D>();
        for(Rectangle2D box: boxes) {
            if(box.getMinX()==min){
                firstColumn.add(box);
            }
    ...
    private double findMinimumY(){
        ArrayList<Rectangle2D> firstColumn = getFirstColumnBoxes();
        double min=100000;
        for(Rectangle2D box: firstColumn) {
            if(box.getMinY()<min){
                min=box.getMinY();
            }
        }
    ...
    private void removeJunk(){
        double minY = findMinimumY();
        double maxY = findMaximumY();

        boxes.removeIf(box -> box.getMinY() < minY || box.getMaxY() > maxY);
    }
